...
lp = 'G:\\SHN-FTP-XFR\\IS-APPS\\RCPSI\\JDA_837'
test_lp = 'c:\\test\\test'
#lp = test_lp 
...
class Prcsr():
    def __init__(self):
        ...
        self.build_sites()
        self.getx()
        for so in self.src_objs:
            self.rename_jda_filename(so)
        self.write_zips()
        self.copy_move_zips()
        self.end()
    ...
    def copy_move_zips(self):
        for zfn,dt in self.zfs.items():
            try:
                ...
                copy(zf,lp)
                arc_p = os.path.join(arc_base_p,dt.strftime('%Y'))
                new_z = os.path.join(arc_p,zfn)
                os.renames(zf,new_z)
            except:
